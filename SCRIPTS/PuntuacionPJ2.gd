...
func _mostrar_flotante(cantidad: int, pos_mundo: Vector2) -> void:
	if cantidad == 0:
		return
	# Siempre arriba del personaje que consiguió los puntos (P2).
	var pos := pos_mundo
...
func _buscar_jugador() -> Node:
	var pj = get_parent().get_node_or_null("PJ2")
	if pj == null:
		pj = get_tree().current_scene.get_node_or_null("PJ2") if get_tree().current_scene != null else null
	return pj
